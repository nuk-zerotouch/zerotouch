...
# 載入處理後的數據
npz_file_path = r'C:\Users\72161\OneDrive\文件\GitHub\k60168a-dongle-utilities\radar-gesture-recognition\src\data\processed_data\test_dataset.npz'
...
def get_max_label_interval(label):
    ...
    return max(max_interval, current_interval)
...
def main():
    ...
    data_length = len(labels)  # 動態獲取資料的長度
    while True:
        user_input = input(f"Enter an index (0-{data_length - 1}) to plot or 's' to stop: ").strip()
        ...
        try:
            ...
            if 0 <= index < data_length:
                ...
                print(f"Index out of range. Please enter a number between 0 and {data_length - 1}.")
        except ValueError:
